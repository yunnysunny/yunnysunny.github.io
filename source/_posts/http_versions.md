...
但是由于上述规范的缺陷过大，并且在客户端实现起来有一定不确定性，所以现在没有任何一个浏览器厂家采用上述方案。

> 这种行为在 HTTP 协议中被称之为 **HTTP Pipelining**，虽然各大浏览器厂商没有支持，但是苹果的 HLS 协议中对其做了支持，因为 HLS 属于流式协议，和上述 Pipelining 的模式比较匹配。

在复用单个 TCP 句柄的时候，都必须串行发送，一条请求的响应接收到之后才能发送下一条请求。所以最终在单条 TCP 句柄上的请求还是变成下面这个模样：
